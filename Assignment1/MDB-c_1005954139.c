 ...
 *
 * Student Name (Last name, First name): Nain, Zumran
 * Student Number: Zumran Nain
 * UTORid: 1005954139
 ...
 *
 * Sign here with your name: Zumran NAin
 *
 *
 */
...
{
	MovieReview review; //One review of size movie review
	struct reviewNode_struct *next; // pointer to the next entry in linked list
...
ReviewNode *newMovieReviewNode(char *title, char *studio, int year, double BO_total, int score)
{
    ...
    /***************************************************************************/
	ReviewNode *new_review = NULL; // pointer to new node

	new_review = (ReviewNode *)calloc(1, sizeof(ReviewNode)); //returns a pointer without any attached type
	// initialize the new codes content with values that shows it has not been filled.
	int boolean = 1;
	if(new_review == NULL){return NULL;} // if linked list is empty return NULL

	strcpy(new_review->review.movie_title, title); // Use paramters to initalizaee data
	strcpy(new_review->review.movie_studio, studio);
	if (year >= 1920 &&  year <= 2999)
	{
...
ReviewNode *findMovieReview(char *title, char *studio, int year, ReviewNode *head)
{
    ...
    /**********  TODO: Complete this function *********************************/
    /***************************************************************************/
	// carries a Carries out a list traversal, looking for a review for a movie with matching fields,
// ll three fields must be a match.
// If a matching movie review is found, it returns a pointer to the node that contains it,
// otherwise it returns NULL.
	...
	while (p!= NULL)
	{
		...
		{
			return p; // found the key
		}
		//Update the traversal pointer to point to the next value
		p = p->next;

	}

    return NULL;  
	// search key wasnt found
}
...
ReviewNode *insertMovieReview(char *title, char *studio, int year, double BO_total, int score,
    ...
    /**********  TODO: Complete this function **********************************/
    /***************************************************************************/
	ReviewNode *new_node = NULL; // Traversal pointer

	ReviewNode *movie_to_find = findMovieReview(title, studio, year, head);
	if (movie_to_find == NULL)
	{
		new_node = newMovieReviewNode(title, studio, year, BO_total, score); // create a new node
		new_node->next = head; // make the nexxt node point to head
		strcpy(new_node->review.movie_title, title); // intialize the title
		strcpy(new_node->review.movie_studio,studio); //intialize the studio
		new_node->review.year = year; //intialize the year
		new_node->review.BO_total = BO_total; //intialize the BO_total
		new_node->review.score = score; // intialize score
		return new_node;
	}
	else // if movie is already in the list
	{
		printf("Sorry,that movie already exists\n");
		return head; // return current head idk how to do that
	}
}
...
int countReviews(ReviewNode *head)
{
	...
	ReviewNode *p = NULL; //initialize pointer to NULL as always

	p = head; // set it to the pointer

	while (p!= NULL) //traverse through entire list
	{
		
		length = length + 1;
		p = p->next; // update traversal pointer
                       ...
                       ReviewNode *head)
{
	...
	ReviewNode *movie_to_find  = findMovieReview(title, studio, year, head); // traverse throught he list using helper

	if (movie_to_find == NULL)// if such a movie is found
	{
		printf("Sorry that movie does not seem to be in the file!\n");

	}
	else{
		movie_to_find->review.score = score; // update
		movie_to_find->review.BO_total = BO_total;
	}
}
...
ReviewNode *deleteMovieReview(char *title, char *studio, int year, ReviewNode *head)
{
    ...
    /**********  TODO: Complete this function *********************************/
    /***************************************************************************/
	// THis functions removes the node from the link lis that contains the review with a matching restaurant name.

	ReviewNode *tr = NULL;
	ReviewNode *pre = NULL;


	if(head == NULL) return NULL; // we have a empty linked list
	// set up pre and tr pointers to point to the first two nodes in the list
...
double printMovieReviews(ReviewNode *head)
{
	...
	p = head; //Initial pointer
	
	double new_total = 0;

	while (p != NULL)
	{
		// Print our the review at this node
		 printf("**********************************************\n");
		 printf("%s\n", p->review.movie_title);
		 printf("%s\n", p->review.movie_studio);
		 printf("%d\n", p->review.year);
		 new_total += p ->review.BO_total;
		 printf("%f\n", new_total);
...
double queryReviewsByStudio(char *studio, ReviewNode *head)
{
	...
	ReviewNode *p = NULL; //intialize pointer to null
	p = head; // ppoint to the head

	 // user input gets studio

	while(p != NULL)
	{
		if (strcmp(p->review.movie_studio, studio) == 0){ // compare the pointer studio to the user input studio
		 printf("**********************************************\n");
		 printf("%s\n", p->review.movie_title);
		 printf("%s\n", p->review.movie_studio);
		 printf("%d\n", p->review.year);
		 ...
		 p = p->next;// update the traversal pointer to point to the next node
...
double queryReviewsByScore(int min_score, ReviewNode *head)
{
	...
	while(p != NULL) //While the pointer is NULL
	{
		if (p->review.score >= min_score){ //if the review socre is greater than the minimum score
		 printf("**********************************************\n");	
		 printf("%s\n", p->review.movie_title);		 
		 printf("%s\n", p->review.movie_studio);
		 printf("%d\n", p->review.year);
...
void insertCastMember(char *title, char *studio, int year, ReviewNode *head, char *name)
{
	...
	ReviewNode *movie_to_find;
	CastList *new_cast;
	
	movie_to_find = findMovieReview(title, studio, year, head);
	if (movie_to_find == NULL)
	{
		printf("There does not exist such a move\n");
	}
	else
		//insert the linked cast list here.
	new_cast = (CastList *)calloc(1, sizeof(CastList));
    strcpy(new_cast->name, name);
	
	 new_cast->next = movie_to_find->review.cast;

	movie_to_find->review.cast = new_cast; // cast is pointer make it point to new cast
}
