...
// Check that the head node contains the new review.
   fprintf(stderr, "****%f\n", MDB_head->review.BO_total);
     ...
     // Check linkage. The new head node's *next field should contain a pointer     // to the old head of the list (which we conveniently saved)
...
// 
   ...
   //  Expected: We should find the cast member's name at the first entry of the linked  list for the cast.    
