...
BST_Node *newBST_Node(double freq, int bar, double index)
{
     ...
     ****/
	 ...
	 new_node->freq = freq;
	 new_node->bar= bar; // intialize it with values that can never occure
	 new_node->index= index;
	 new_node->left = NULL; // remember to intialize left and right pointers to NULL
	 new_node->right = NULL;
	 
	 
    return new_node; //dont forget to return the new node
}
...
BST_Node *BST_insert(BST_Node *root, BST_Node *new_node)
     ...
     ****/
	 {	
	 if (root == NULL){
		 return new_node; //tree is empty, new node becomes the root
	 }
...
BST_Node *BST_search(BST_Node *root, int bar, double index)
{
	...
	//not in this node, search teh subtree 
...
BST_Node *BST_delete(BST_Node *root, int bar, double index)
{
     ...
     ****/
	 BST_Node *tmp;
	 double key = (10.0*bar)+index;
     if(root == NULL) return NULL; //tree or subtree is empty
	 // check if this si the node we want to delete_BST
	 
	 if (root->key == key){
		 if(root->left == NULL && root->right == NULL){
		//case a with 0 chileren
			free(root);
			return NULL;
		}	
		else if(root->right == NULL){
		//case b only the left child has to be linked-
			tmp = root->left;
			free(root);
			return tmp;	
		}
		else if(root->left == NULL){
		//case b only one child but on teh right
			tmp = root->right;
			free(root);
			return tmp;	
		}
	 ...
	 //not in this node, dekete the corresponding subtree and
	 //update the corescponding linked-
	 if(key < root->key ){
...
void BST_makePlayList(BST_Node *root)
{
     ...
     ****/
	  if (root == NULL) return;
	 {
		 BST_makePlayList(root->left); //traverse leftsubtreee
		 playlist_head = playlist_insert(playlist_head, root->freq, root->bar, root->index); //insert the playlist
		 BST_makePlayList(root->right); //traverse right subtree
	 }

}
double nameToFreq(char note_name[5])
{
	/* This function changes the note name to its corresponding
	 * frequency.
	 */
	 for (int i=0; i < 100; i++)
	 {
...
void BST_inOrder(BST_Node *root, int depth)
{
     ...
     ****/
	 ...
	 if (root != NULL){ 
	 BST_inOrder(root->left, depth+1); // traverse left subtree in order
	 printf("Depth = %d, Bar: Index (%d: %f), F = %f Hz\n", depth, root->bar, root->index, root->freq); //print the data at the node
	 BST_inOrder(root->right, depth+1); // traverse the right subtree in order
...
void BST_preOrder(BST_Node *root, int depth)
{
     ...
     ****/
	 ...
	 printf("Depth = %d, Bar: Index (%d: %f), F = %f Hz\n", depth, root->bar, root->index, root->freq); //print the data at the node
	 BST_preOrder(root->left, depth+1); // traverse left subtree in pre-order
	 BST_preOrder(root->right, depth+1); // traverse the right subtree in pre-order
	 }
...
void BST_postOrder(BST_Node *root,int depth)
{
     ...
     ****/
	  ...
	  if (root != NULL){  // if list is empty return NULL
	 BST_postOrder(root->left, depth + 1); // traverse left subtree in post-order
	 BST_postOrder(root->right, depth + 1); // traverse the right subtree in post-order
	printf("Depth = %d, Bar: Index (%d: %f), F = %f Hz\n", depth, root->bar, root->index, root->freq); //print the data at the node
	  }
...
void delete_BST(BST_Node *root)
{
     ...
     ****/
	 ...
	 delete_BST(root->left); // delete the left subtree in post-order;
	 delete_BST(root->right); //delete the right subtree in post-order;
	 free(root); // apply operation on root
...
void BST_shiftFreq(BST_Node *root, char note_src[5], char note_dst[5])
{
     ...
     ****/
	 double src = nameToFreq(note_src);
	 double dst = nameToFreq(note_dst);
	 
	 if (root == NULL) return;
	 {
		 BST_shiftFreq(root->left, note_src, note_dst); //traverse the left subree
		 if (src == root->freq){ //if the source frequency matches the node frequency
			 root->freq = dst; // the frequency is updated to the new one
		 }
		 BST_shiftFreq(root->right, note_src, note_dst); // traverse right sub-tree
	 }
	 

}
double convertFreq(double freq, int semitones)
{
	/* Return the new frequency of a note given
	the frequency and semitones.
	 */
	 for (int j=0; j<100; j++) //loop through the semitones(max of 99)
	 {
		 if ((j+semitones >= 0)&&(j+semitones < 100)&& (note_freq[j]==freq)) //checks that  the frequency is in range
		 {
			 return note_freq[j+semitones]; //return the frequency plus semitones
		 }
	 }
	 return -1;
}
 ...
 * ******************************************************************************************/
 BST_Node *BST_duplicate(BST_Node *root, BST_Node *new_node)
     ...
     ****/
	 ...
	 if (root == NULL){
		 root = new_node;
		 return root; //tree is empty, new node becomes the root
	 }
	 //determine which subtree the new key should be in
	 if (new_node->key <root->key){
		 ...
		 root->index = root->index + 0.001;
		 root->key = root->key + 0.001;
...
BST_Node *BST_harmonize(BST_Node *root, int semitones, double time_shift)
{
	...
	if (root != NULL)
	{
		root->left = BST_harmonize(root->left, semitones, time_shift); //traverse the left subtree to modify frequencies
		root->right = BST_harmonize(root->right, semitones, time_shift); // traverse the right subtree to modify frequencies
		double convertfreq = convertFreq(root->freq, semitones); //helper function to convert frequency using semitones
		if (((root->index)+ time_shift<1) && (0 <= (root->index)+ time_shift) && (convertfreq!= -1)){ // if the frequency matches the conditions
		
			new_node = newBST_Node(convertfreq,root->bar, root->index+time_shift);
			
			root = BST_duplicate(root, new_node); // insert the new node at the root
